...
# 1. 網頁基本設定 
st.set_page_config(page_title="三刀流全能戰情室 (PRO版)", page_icon="⚔️", layout="centered")
...
if df.empty:
    ...
    # 4. 根據不同資產屬性，動態調整「超跌甜甜價」門檻
    if asset_type == "一般個股":
        sweet_threshold = -15.0  # 個股波動大，負乖離 15% 算甜甜價
        stop_loss_pct = 0.97     # 季線下方 3% 停損
    else:
        sweet_threshold = -5.0   # ETF 波動小，負乖離 5% 就算甜甜價
        stop_loss_pct = 0.98     # 季線下方 2% 停損
    ...
    if stable_above_60 and ma20 > ma60:
        ...
        elif current_vol > vol_5ma and macd_hist_val > 0:
        ...
        if current_price > ma60 and not stable_above_60:
            action_signal = "👀【假突破警戒】今天站上季線但尚未過三日測試，提防主力騙線。"
            signal_color = "warning"
        else:
            action_signal = "⏳【盤整中】多空拉鋸，耐心等候明確突破或甜甜價降臨。"
            signal_color = "warning"
    ...
    if bias_60 <= sweet_threshold:
        st.success(f"💎 **【黃金甜甜價觸發】** 負乖離達 {bias_60:.1f}%（已跌破甜蜜門檻 {sweet_threshold}%）！估算甜甜價位約在 **{calculated_sweet_price:.2f}** 以下，可考慮分批佈局。")
    elif bias_60 >= (15 if asset_type == "一般個股" else 6):
        st.error(f"🚨 **【短線過熱警告】** 正乖離達 {bias_60:.1f}%，短線乖離過大，慎防回檔！")
    else:
        st.info(f"🎯 **甜甜價雷達監控中**：當前乖離 {bias_60:.1f}%（觸發甜甜價門檻：{sweet_threshold}% / 參考價：約 {calculated_sweet_price:.2f}）")
    ...
    # 底層數據儀表板
    macd_status = "🟢 多方動能增強" if macd_hist_val > 0 else "🔴 空方動能主導"
    st.markdown(f"""
    > 📊 **多維度底層數據**  
